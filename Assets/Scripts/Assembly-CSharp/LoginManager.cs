...
public class LoginManager : MonoBehaviour
{
    ...
    private void OnLevelWasLoaded()
    {
        ...
        string udid = OpenUDIDBinding.GetOpenUDID();
        if (udid == null)
            udid = "super_guest_common_to_all";

        string hash = Bootloader.Instance.GetMD5Hash(udid);
        currentUsername = GUEST_ACCOUNT_PREFIX + hash + "droid";
        currentPassword = Bootloader.Instance.GetMD5Hash(currentUsername);
    ...
    public void OnLogin()
    {
        Debug.Log("Forced login success for user: " + currentUsername);
